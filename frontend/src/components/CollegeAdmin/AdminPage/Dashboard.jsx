            ...
            {jobPostings.length > 0 ? (
              jobPostings.map((job, index) => (
                <li key={index} className="p-4 border-b last:border-b-0 hover:bg-white transition-all">
                  <p className="text-lg font-medium text-slate-800">{job.title} at {job.company}</p>
                  <p className="text-sm text-gray-500">Posted on: {formatDateInIST(job.created_at)}</p>
                </li>
              ))
            ) : (
            ...
            {placements.length > 0 ? (
              placements.map((placement, index) => (
                <li key={index} className="p-4 border-b last:border-b-0 hover:bg-gray-100 transition-all">
                  <p className="text-lg font-medium text-slate-800">{placement.student_name}</p>
                  <p className="text-sm text-gray-500">
                    Placed at {placement.company} as {placement.job_position} on {placement.updated_at}
                  </p>
                </li>
              ))
            ) : (
