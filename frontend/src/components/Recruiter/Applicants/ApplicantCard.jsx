...
const ApplicantCard = ({ form }) => {
  ...
  const UpdateAppStatus = async (Status) => {
    ...
    setLoading(true);
    try {
      const response = await axios.put("http://127.0.0.1:5000/job-applications",
        {
          id: id,
          status: Status,
        },
    ...
    <div className="bg-white rounded-xl shadow-lg overflow-hidden border-l-4 border-blue-500 flex flex-col min-w-96 mx-4">
      ...
      <div className="flex mt-auto">
        ...
        {status === "Placed" || status === "Rejected" ? (
          ...
          <>
            <button onClick={() => UpdateAppStatus("Placed")} className="flex-1 py-4 px-4 bg-green-600 text-white font-semibold shadow-md hover:bg-green-400 focus:outline-none cursor-pointer">
              Hire
            </button>
            <button onClick={() => UpdateAppStatus("Rejected")} className="flex-1 py-4 px-4 bg-red-500 cursor-pointer text-white font-semibold shadow-md hover:bg-red-600 focus:outline-none">
              Reject
            </button>
          </>
        )}
