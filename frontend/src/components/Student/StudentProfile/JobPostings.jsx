...
const JobPostings = () => {
  ...
  const fetchPostings = async () => {
    ...
    try {
      ...
      if (response.status === 200) {
        ...
        const filteredJobs = jobs.filter((job) => job.eligibility && job.eligibility.length > 0);
  ...
  return (
    <div className="p-6">
      <h2 className="text-2xl font-semibold mb-6 text-left">Job Postings</h2>
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
        {availJobPostings.map((job, index) => (
          ...
          >
            <h3 className="text-xl font-semibold text-gray-800 mb-4">{job.company}</h3>
            ...
            <p className="text-gray-600 mb-4">
              Deadline: <span className="font-semibold">{formatDateInIST(job.application_deadline)}</span>
            </p>
            <button
      ...
      {selectedJob && <JobApplicationForm job={selectedJob} onClose={handleCloseForm} />}
    </div>
  )
}
