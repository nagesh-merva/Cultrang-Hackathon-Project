...
import RoundForm from '../../components/Recruiter/Rounds/RoundForm';

function Rounds() {
  const [rounds, setRounds] = useState([
    // Sample rounds for initial state
    { id: '1', name: 'Round 1', type: 'Type A', status: 'pending', date: '2025-01-20', notes: 'Some notes here' },
    { id: '2', name: 'Round 2', type: 'Type B', status: 'completed', date: '2025-01-21', notes: 'Some more notes here' }
  ]);
        ...
        {rounds.map((round) => (
          <div key={round.id} className="bg-white shadow rounded-lg p-6 mb-4">
            <div className="flex justify-between items-center mb-4">
              <div>
                <h3 className="text-2xl font-medium text-gray-900">{round.name}</h3>
              ...
              <div className="flex items-center space-x-4">
                ...
                >
                  {round.status.charAt(0).toUpperCase() + round.status.slice(1)} {/* Capitalizing status */}
                </span>
                {round.status !== 'completed' && (
                  <button
